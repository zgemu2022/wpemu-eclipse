...
#define SWAP(a,b) { unsigned long t = a; a = b; b = t; t = 0; }
...

		{
			...
			c = bb ^ (ROTL(bb, 2)) ^ (ROTL(bb, 10)) ^ (ROTL(bb, 18))
					^ (ROTL(bb, 24));
...
// #endif
		...
		void our_sm4_d(unsigned char *pbuf,int *plen,unsigned char* key_sm4)
		{
			sm4_context ctx;
			int i,num;
			int len=*plen;
			unsigned char buf_temp[1024];
			...
			printf("g_nwpara.key_sm4=%s\n",key_sm4);
			sm4_setkey_dec(&ctx,key_sm4);//g_nwpara.key_sm4
			sm4_crypt_ecb(&ctx,0,len,pbuf,buf_temp);
			printf("解密数据:len=%d\n",len);
			for(i=0;i<len;i++)
			{
				pbuf[i]=buf_temp[i];
		...
		void our_sm4_e(unsigned char *pbuf,int *plen,unsigned char* key_sm4)
		{
			sm4_context ctx;
			int i,num;
			int len1=*plen,len2,len;
			unsigned char buf_temp[1024];
			...
			len=16*num;
			memset(&buf_temp[0],0,1024);

			memcpy(&buf_temp[0],pbuf,len1);

			//encrypt standard testing vector
			printf("g_nwpara.key_sm4=%s\n",key_sm4);//g_nwpara.key_sm4
			sm4_setkey_enc(&ctx,key_sm4);
			sm4_crypt_ecb(&ctx,1,len,buf_temp,buf_temp);
